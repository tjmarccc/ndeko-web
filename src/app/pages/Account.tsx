...
function SuccessBanner({ message }: { message: string }) {
  return (
    <div className="flex items-center gap-3 p-4 rounded-xl border border-emerald-200 bg-emerald-50 dark:bg-emerald-900/10 dark:border-emerald-900/30 text-sm text-emerald-700 dark:text-emerald-400">
...
function ConfirmDialog({
  title, description, confirmLabel, requireText, loading, error, onConfirm, onCancel,
}: {
  title: string; description: string; confirmLabel: string; requireText: string;
  loading: boolean; error: string; onConfirm: () => void; onCancel: () => void;
}) {
  const [input, setInput] = useState('');
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onCancel}>
      <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-sm shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
        <h3 className="font-bold text-lg dark:text-white mb-2">{title}</h3>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{description}</p>
        {error && <div className="mb-3 text-sm text-red-600 dark:text-red-400">{error}</div>}
        <p className="text-xs text-gray-500 mb-1">Type <strong>{requireText}</strong> to confirm</p>
        <Input
          value={input}
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={onCancel} className="flex-1 dark:border-gray-700 dark:text-gray-300">
            Cancel
          </Button>
          <Button
            onClick={onConfirm}
            disabled={input !== requireText || loading}
            className="flex-1 bg-red-600 hover:bg-red-700 flex items-center gap-2 justify-center"
          >
