...
function PriceRangeSlider({
  ...
  function makeDragHandler(isMin: boolean) {
    return (eDown: React.MouseEvent | React.TouchEvent) => {
      ...
      // Capture current value at drag start so closure is stable
      ...
      const onMove = (e: MouseEvent | TouchEvent) => {
        ...
        const v = valFromX(cx);
        if (isMin) {
          onChange([Math.max(min, Math.min(v, startHi - step)), startHi]);
        } else {
          onChange([startLo, Math.min(max, Math.max(v, startLo + step))]);
        }
      };
